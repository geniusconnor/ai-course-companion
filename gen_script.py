...
# ── 数据定义 ──────────────────────────────────────────────
# 段落结构：(seq, name, function, duration, shots, sync_voice, edit_notes)
# shots = list of (shot_no, shot_desc)  可能有多个镜头
segments = [
    {
        "seq": 1,
        "name": "先感受她"停不下来"",
        "function": "让观众身体上感受到：她是一直在动、停不下来、能量特别满的小孩。先别解释她是谁。",
        "duration": "约 15–20 秒（整体 50 秒–1 分 10 秒）",
        "shots": [
            ("1-A", "在家骑平衡车"),
            ("1-B", "在家里跑跳的画面\n（后期补：厨房爬墙视频）"),
            ("1-C", "拿到蛋糕时兴奋反应"),
        ],
        "sync_voice": ""我没有安静的时候，只有睡觉。"\n\n→ 尽量早出，甚至可做开场第一句",
        "edit_notes": (
            "・节奏快，多用她在动的镜头\n"
            "・这句台词尽量早出，可做片头第一句\n"
            "・不要加解说，靠画面让观众先「感受到」"
    ...
    },
    {
        "seq": 2,
        "name": "进入她的世界",
        "function": "让观众知道：她有一个很明确的世界中心。不是展示优秀，而是她已经很清楚自己最重要的东西是什么。",
        "duration": "15–20 秒",
        "shots": [
            ("2-A", "发小天才手表朋友圈"),
            ("2-B", "奖牌盒子（她与盒子互动）"),
    ...
    },
    {
        "seq": 3,
        "name": "她有自己的语言系统",
        "function": "把她从「有活力的小孩」升级成有边界、有判断、有自己表达方式的人。她会自己命名自己，也会拆别人给她的词。",
        "duration": "15–20 秒",
        "shots": [
            ("3-A", "互贴标签"),
            ("3-B", "妈妈给她贴标签"),
    ...
    },
    {
        "seq": 4,
        "name": "身体状态把人物做实",
        "function": "告诉观众：她的「停不下来」不是嘴贫、不是表演感，是她整个人的真实状态——脑子和身体都在往外长。",
        "duration": "10–15 秒",
        "shots": [
            ("4-A", "攀岩训练时累到手虚"),
            ("4-B", "表情明显疲惫"),
            ("4-C", "休息一下又立刻恢复"),
            ("4-D", "训练结束后还想再爬一条（可选短镜头/声音）"),
        ],
        "sync_voice": (
            "可选旁白：\n"
...
for seg in segments:
    ...
    for i, (shot_no, shot_desc) in enumerate(seg["shots"]):
        # 镜头行
        ws.cell(row=row, column=5, value=shot_no).border = THIN_BORDER
        ws.cell(row=row, column=6, value=shot_desc).border = THIN_BORDER
        ws.cell(row=row, column=5).alignment = CENTER
        ws.cell(row=row, column=6).alignment = WRAP

        # 段落合并列（第一行写内容，其余留空等合并）
...
logic_text = (
    "【整体逻辑】先感受到她停不下来 → 再看到她有自己的世界 → 再发现她很有自己 → 最后用身体状态把这一切坐实｜"
    "不按时间顺序，按观众认识她的顺序剪｜目标：先让观众喜欢她、记住她，而不是先讲清楚她会攀岩"
)
